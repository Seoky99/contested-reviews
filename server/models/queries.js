...
async function getAllSetReviews(userid) {
    ...
    const query = `SELECT user_sets.name AS set_review_name, set_code, sets.name AS set_name FROM user_sets JOIN sets ON user_sets.set_id = sets.set_id 
                    WHERE user_id = $1`;
...
export default { getAllSetReviews, createSetReview };